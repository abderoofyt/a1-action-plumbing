...
def pricing_detail_view(request, id):
    context = {"data": Profile.objects.filter(user=1),"pricing": Pricing.objects.get(id=id), "form": OrderForm()}
...
def orders(request):
    context = { "data": Profile.objects.filter(user=1),"dataset": Order.objects.all()}
